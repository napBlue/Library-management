...
public class TokenUtils {            //token,即前端传过来的一个值，后端要拿来做校检
    ...
    public static Admin getCurrentAdmin() {
        ...
        try {
            ...
            if (StrUtil.isBlank(token)) {  // header没有  那就从参数获取，如admin?token=xxx
            ...
            String adminId = JWT.decode(token).getAudience().get(0); //解开token后获取admin的id
            return staticAdminService.getById(Integer.valueOf(adminId)); //通过adminId获得admin的信息
        } catch (Exception e) {
